...
///
/// Validation is performed automatically on deserialization. Invalid filters
/// will fail to deserialize rather than silently producing a malformed struct.
...
/// Raw deserialization helper for BloomFilter validation.
#[derive(Deserialize)]
struct BloomFilterRaw {
    bits: String,
    num_bits: usize,
    num_hashes: usize,
    count: usize,
...
impl<'de> Deserialize<'de> for BloomFilter {
    ...
    where
        D: Deserializer<'de>,
    {
        let raw = BloomFilterRaw::deserialize(deserializer)?;
        ...
        if !filter.is_valid() {
            return Err(serde::de::Error::custom("invalid bloom filter"));
        }
...
impl BloomFilter {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        for hash in iter {
            // Fresh filter from new() is always valid
            let inserted = filter.insert(hash);
            debug_assert!(inserted, "insert failed on fresh filter");
        }
        filter
    }
    ...
    ///
    /// Returns `true` if insertion succeeded, `false` if the filter is invalid
    /// (corrupted base64, mismatched sizes, etc.). Invalid filters should be
    /// discarded and recreated.
    ///
    /// Note: Count is only incremented if at least one new bit was set.
    /// This prevents count inflation from duplicate insertions.
    pub fn insert(&mut self, hash: &str) -> bool {
        // Validate filter state before modifying
        if self.num_bits == 0 {
            return false;
        }

        let mut bits_vec = match B64.decode(&self.bits) {
            Ok(v) => v,
            Err(_) => return false, // Corrupted base64, refuse to operate
        };

        // Verify bits length matches expected size
        let expected_bytes = self.num_bits.div_ceil(8);
        if bits_vec.len() != expected_bytes {
            return false; // Mismatched size, refuse to operate
        }

        // Track whether we set any new bits (to avoid count inflation on duplicates)
        let mut any_new_bit = false;
        ...
        for i in 0..self.num_hashes {
            ...
            if byte_index < bits_vec.len() {
                ...
                if (bits_vec[byte_index] & mask) == 0 {
                    any_new_bit = true;
                }
                bits_vec[byte_index] |= mask;
            }
        }

        self.bits = B64.encode(&bits_vec);

        // Only increment count if we actually added new information
        if any_new_bit {
            self.count += 1;
        }
        true
    ...
    fn hash_index(&self, hash: &str, func_index: usize) -> usize {
        // Guard against division by zero from malformed/deserialized filters
        if self.num_bits == 0 {
            return 0;
        }
    ...
    /// Validate filter integrity after deserialization.
    /// Returns false if the filter is malformed.
    pub fn is_valid(&self) -> bool {
        ...
        let Ok(bits_vec) = B64.decode(&self.bits) else {
            return false;
        };
        // Verify bits length matches num_bits (with rounding for byte alignment)
        let expected_bytes = self.num_bits.div_ceil(8);
        bits_vec.len() == expected_bytes
    }
}
...
mod tests {
    ...
    fn zero_num_bits_does_not_panic() {
        // Simulate malformed deserialized filter with num_bits = 0
        let malformed = BloomFilter {
            bits: B64.encode([0u8; 8]),
            num_bits: 0,
            num_hashes: 1,
            count: 0,
        ...
        // Should not panic, should return false (not found)
        assert!(!malformed.might_contain("any_hash"));
    }

    #[test]
    fn is_valid_rejects_zero_num_bits() {
        let malformed = BloomFilter {
            bits: B64.encode([0u8; 8]),
            num_bits: 0,
            num_hashes: 1,
    ...
    #[test]
    fn is_valid_rejects_bad_num_hashes() {
        let too_many = BloomFilter {
            bits: B64.encode([0u8; 8]),
            num_bits: 64,
            num_hashes: 100, // exceeds max of 16
            count: 0,
        };
        assert!(!too_many.is_valid());

        let zero_hashes = BloomFilter {
            bits: B64.encode([0u8; 8]),
            num_bits: 64,
            num_hashes: 0,
            count: 0,
        };
        assert!(!zero_hashes.is_valid());
    }

    #[test]
    fn is_valid_rejects_mismatched_bits_length() {
        let mismatched = BloomFilter {
            bits: B64.encode([0u8; 4]), // 4 bytes = 32 bits
            num_bits: 64,               // claims 64 bits
            num_hashes: 1,
            count: 0,
        };
    ...
    #[test]
    fn is_valid_accepts_valid_filter() {
        let config = BloomConfig::default();
        let mut filter = BloomFilter::new(100, &config);
    ...
    #[test]
    fn insert_fails_on_corrupted_base64() {
        let mut corrupted = BloomFilter {
            bits: "not-valid-base64!!!".to_string(),
            num_bits: 64,
            num_hashes: 1,
            count: 0,
        };
        assert!(!corrupted.insert("test"));
        // Count should not increase
        assert_eq!(corrupted.count, 0);
    }

    #[test]
    fn insert_fails_on_mismatched_size() {
        let mut mismatched = BloomFilter {
            bits: B64.encode([0u8; 4]), // 4 bytes
            num_bits: 128,              // claims 16 bytes
            num_hashes: 1,
            count: 0,
        };
        assert!(!mismatched.insert("test"));
        assert_eq!(mismatched.count, 0);
    }

    #[test]
    fn insert_fails_on_zero_bits() {
        let mut zero = BloomFilter {
            bits: B64.encode([]),
            num_bits: 0,
            num_hashes: 1,
            count: 0,
        };
        assert!(!zero.insert("test"));
    }
    ...
    fn duplicate_insert_does_not_inflate_count() {
        ...
        // Insert same hash multiple times
        assert!(filter.insert("same_hash"));
        assert_eq!(filter.count(), 1);

        assert!(filter.insert("same_hash"));
        assert_eq!(filter.count(), 1, "count should not increase for duplicate");

        assert!(filter.insert("same_hash"));
        assert_eq!(filter.count(), 1, "count should not increase for duplicate");

        // Different hash should increase count
        assert!(filter.insert("different_hash"));
        assert_eq!(filter.count(), 2);
    }

    #[test]
    fn deserialize_rejects_invalid_filter() {
        // Malformed: num_bits = 0
        let json = r#"{"bits":"AAAAAAAAAAA=","num_bits":0,"num_hashes":1,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_err(), "should reject num_bits=0");

        // Malformed: num_hashes = 0
        let json = r#"{"bits":"AAAAAAAAAAA=","num_bits":64,"num_hashes":0,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_err(), "should reject num_hashes=0");

        // Malformed: num_hashes > 16
        let json = r#"{"bits":"AAAAAAAAAAA=","num_bits":64,"num_hashes":100,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_err(), "should reject num_hashes>16");

        // Malformed: bits length doesn't match num_bits
        let json = r#"{"bits":"AAAA","num_bits":64,"num_hashes":1,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_err(), "should reject mismatched bits length");

        // Malformed: invalid base64
        let json = r#"{"bits":"not-valid!!!","num_bits":64,"num_hashes":1,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_err(), "should reject invalid base64");
    }

    #[test]
    fn deserialize_accepts_valid_filter() {
        // Valid filter: 8 bytes = 64 bits
        let json = r#"{"bits":"AAAAAAAAAAA=","num_bits":64,"num_hashes":3,"count":0}"#;
        let result: Result<BloomFilter, _> = serde_json::from_str(json);
        assert!(result.is_ok(), "should accept valid filter");
    }
}
